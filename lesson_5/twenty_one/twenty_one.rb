...
def calculate_ace_values(number_of_aces, total)
...
def sum_cards_values(participant)
  ...
  participant.each do |card|
    if card == 'Ace'
      number_of_aces += 1
    elsif card.to_i.zero? # It's a String, so it's value is 10
      total += 10
    else
      total += card.to_i
    end
  ...
  total = calculate_ace_values(number_of_aces, total)
end
...
puts ""
prompt "Press any key to deal the cards..."
gets
